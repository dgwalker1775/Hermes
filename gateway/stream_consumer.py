...
class GatewayStreamConsumer:
    ...
    # Detects when the LLM enters a runaway repetition loop (context overflow
    # or stuck reasoning). Once a phrase of at least _REPEAT_MIN_LEN characters
    # appears _REPEAT_COUNT times consecutively in the recent output window,
    # the stream is hard-aborted with an error message so the user isn't flooded.
    _REPEAT_MIN_LEN: int = 80       # minimum phrase length to consider a repeat
    _REPEAT_COUNT: int = 3          # how many times a phrase must repeat to abort
    _REPEAT_WINDOW: int = 6000      # scan only the trailing N chars of accumulated

    @staticmethod
    def _is_repetition_candidate(phrase: str) -> bool:
        """Reject degenerate candidates like a single repeated character."""
    ...
    def _check_repetition_loop(self) -> bool:
        """Return True if a runaway repetition is detected in accumulated output.

        Uses a sliding-window search: for each candidate phrase length (from
        _REPEAT_MIN_LEN up to window//(_REPEAT_COUNT+1)), check whether the
        phrase at the end of the window repeats _REPEAT_COUNT times consecutively
        just before the final occurrence.
        """
        tail = self._accumulated[-self._REPEAT_WINDOW:]
        n = len(tail)
        max_phrase = n // (self._REPEAT_COUNT + 1)
        if max_phrase < self._REPEAT_MIN_LEN:
            return False
        # Check a handful of candidate lengths rather than every possible size
        for phrase_len in range(self._REPEAT_MIN_LEN, min(max_phrase, 800) + 1, 40):
            phrase = tail[-phrase_len:]
            if not self._is_repetition_candidate(phrase):
                continue
            pos = len(tail) - phrase_len
            repeats = 0
            while pos >= phrase_len and tail[pos - phrase_len:pos] == phrase:
                repeats += 1
                pos -= phrase_len
                if repeats >= self._REPEAT_COUNT:
                    return True
        return False

    def _abort_repetition_loop(self) -> None:
        """Hard-stop a detected repetition loop: truncate output and send error."""
        if getattr(self, "_repetition_aborted", False):
            return
        self._repetition_aborted = True
        logger.error(
            "Repetition loop detected after %d accumulated chars — aborting stream",
            len(self._accumulated),
        )
        # Trim trailing repeated content and append an error notice
        tail = self._accumulated[-self._REPEAT_WINDOW:]
        for phrase_len in range(self._REPEAT_MIN_LEN, min(len(tail) // (self._REPEAT_COUNT + 1), 800) + 1, 40):
            phrase = tail[-phrase_len:]
            if not self._is_repetition_candidate(phrase):
                continue
            pos = len(tail) - phrase_len
            while pos >= phrase_len and tail[pos - phrase_len:pos] == phrase:
                pos -= phrase_len
            if pos < len(tail) - phrase_len:
                # Found the start of the repeating block — trim there
                trim_at = len(self._accumulated) - self._REPEAT_WINDOW + pos + phrase_len
                self._accumulated = self._accumulated[:trim_at]
                self._stream_ledger = self._accumulated
                break
        error_suffix = (
            "\n\n---\n"
            "⚠️ **Response stopped: repetition loop detected.**\n"
            "I ran into a context overflow and started repeating myself. "
            "Please start a new session or ask me to summarize and continue."
        )
        abort_text = self._accumulated.rstrip() + error_suffix
        self._queue.put((_FINAL_TEXT, abort_text))
    ...
    def _append_accumulated(self, text: str) -> None:
        ...
        # Repetition-loop guard: check every 200 chars to avoid hot-path cost
        if (
            not getattr(self, "_repetition_aborted", False)
            and len(self._accumulated) % 200 < len(text) + 1
            and len(self._accumulated) > self._REPEAT_MIN_LEN * self._REPEAT_COUNT
            and self._check_repetition_loop()
        ):
            self._abort_repetition_loop()
