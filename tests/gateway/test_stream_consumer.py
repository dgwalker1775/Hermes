...
def _make_repetition_consumer() -> GatewayStreamConsumer:
    ...
    consumer._repetition_aborted = False
    return consumer
...
class TestRepetitionLoopGuard:
    def test_repetition_loop_detection_fires_on_repeated_block(self):
        consumer = _make_repetition_consumer()
        block = "0123456789" * 8
        consumer._accumulated = block * 4

        assert consumer._check_repetition_loop() is True

    def test_repetition_abort_queues_final_and_done(self):
        consumer = _make_repetition_consumer()
        block = "0123456789" * 8
        consumer._accumulated = block * 4
        consumer._stream_ledger = consumer._accumulated

        consumer._abort_repetition_loop()

        assert consumer._repetition_aborted is True
        assert consumer._queue.put.call_count == 2
        final_kind, final_text = consumer._queue.put.call_args_list[0].args[0]
        assert "repetition loop detected" in final_text.lower()
